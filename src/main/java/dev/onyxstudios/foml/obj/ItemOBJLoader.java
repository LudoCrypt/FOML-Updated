...
/***
 * ItemOBJLoader Child class of the OBJ loader that loads basic item OBJ models
 * with JSON model transformations.
 *
...
public class ItemOBJLoader implements ModelVariantProvider, Function<ResourceManager, ModelVariantProvider> {
	...
	public static final Gson GSON = (new GsonBuilder()).registerTypeAdapter(ModelTransformation.class, new ItemOBJLoader.ModelTransformDeserializer()).registerTypeAdapter(Transformation.class, new ItemOBJLoader.TransformDeserializer()).create();
	...
	public UnbakedModel loadModelVariant(ModelIdentifier modelId, ModelProviderContext context) {
		if (OBJ_LOADER.isRegistered(modelId.getNamespace()) && modelId.getVariant().equals("inventory")) {
			...
			Identifier modelPath = new Identifier(modelId.getNamespace(), "models/item/" + modelId.getPath() + ".json");
			...
			try (Reader reader = new InputStreamReader(resourceManager.getResource(modelPath).getInputStream())) {
				...
				return (OBJUnbakedModel) OBJ_LOADER.loadModelResource(parentPath, context, transformation);
			} catch (Exception e) {
				// Silently ignore general IllegalStateExceptions, as all vanilla models in a
